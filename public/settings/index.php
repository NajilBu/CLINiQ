...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['start_ape_cycle', 'close_ape_cycle', 'archive_ape_cycle', 'reset_school_year', 'create_ape_schedule_batch', 'cancel_ape_schedule_batch'], true)) {
        ...
        try {
            ...
            } elseif ($action === 'create_ape_schedule_batch') {
...
if ($canManageApeCycles) {
    try {
        ...
        if (($apeCurrentCycle['status'] ?? '') === 'Active') {
            $apeScheduleBatches = ape_schedule_batches((int) $apeCurrentCycle['ape_cycle_id']);
            $apeScheduleGroups = ape_schedule_candidate_groups((int) $apeCurrentCycle['ape_cycle_id']);
        }
    } catch (Throwable $e) {
        flash_message('error', $e->getMessage());
    }
                ...
                <div id="settings-ape-cycle" class="settings-tab-panel <?= $currentTab === 'ape-cycle' ? 'active' : '' ?> space-y-6">
                    ...
                    <section class="settings-section space-y-5">
                        <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                            <div>
                                <div class="flex flex-wrap items-center gap-2 mb-1">
                                    <h3 class="font-headline text-lg font-extrabold text-[#17261d]">Scheduled APE Batches</h3>
                                    <span class="badge badge-in-progress"><?= count($apeScheduleBatches) ?> batch<?= count($apeScheduleBatches) === 1 ? '' : 'es' ?></span>
                                </div>
                                <p class="settings-help mb-0">School Year <?= e($apeCurrentCycle['academic_year']) ?> &bull; Schedule Students, Faculty, and School Personnel separately. Different categories cannot overlap at the same date and time.</p>
                            </div>
                            <button type="button" class="btn btn-primary justify-center shrink-0" id="openApeBatchModalButton" <?= $apeScheduleGroups === [] ? 'disabled' : '' ?>>
                        ...
                        $apeSettingsBatchColumns = [
                            ...
                            ['headerName' => 'Actions', 'field' => 'actionHtml', 'cellRenderer' => 'html', 'sortable' => false, 'filter' => false, 'width' => 170, 'minWidth' => 160],
                        ];
                        $apeSettingsBatchRows = [];
                        foreach ($apeScheduleBatches as $batch) {
                            ...
                            $batchActionHtml = '<span class="text-xs font-bold text-slate-400">No action</span>';
                            if (!$batchIsCancelled && !$batchHasPassed) {
                                $batchActionHtml = '<form method="post" data-no-ajax="true">'
                            ...
                            $apeSettingsBatchRows[] = [
                                ...
                                'actionHtml' => $batchActionHtml,
                            ];
                        }
                        render_ag_grid('apeSettingsBatchGrid', $apeSettingsBatchColumns, $apeSettingsBatchRows, [
                            'pageSize' => 10,
                            'pagination' => true,
                            'paginationControls' => 'apeSettingsBatchPagination',
                            'height' => 'compact',
                            'emptyTitle' => 'No APE batches scheduled',
                            'emptyText' => 'Create a patient batch to set examination dates and times.',
                        ]);
                        ?>
                        <nav id="apeSettingsBatchPagination" class="pagination" aria-label="Scheduled APE batch pages"></nav>

                        <?php if ($apeScheduleGroups === []): ?>
                            <p class="settings-help mb-0">All eligible sections, departments, and offices are already assigned, or there are no eligible groups in this cycle.</p>
                        <?php endif; ?>
                    </section>
                    <?php endif; ?>
...
<?php if ($canManageApeCycles): ?>
            ...
            <form method="post" data-no-ajax="true" id="apeBatchForm" class="space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="settings-field mb-0">
                        ...
                        <input class="settings-input" id="apeBatchName" name="batch_name" maxlength="120" placeholder="Student Batch A" required>
                    </div>
                    <div class="settings-field mb-0">
                        <label class="clinic-label" for="apeBatchCategory">Patient Category</label>
                        <select class="settings-input" id="apeBatchCategory" name="patient_category" required>
                    ...
                    <div class="settings-field mb-0">
                        ...
                        <input class="settings-input" id="apeBatchDate" name="schedule_date" type="date" min="<?= e($apeCurrentCycle['compliance_start']) ?>" max="<?= e($apeCurrentCycle['compliance_end']) ?>" required>
                    </div>
                    <div class="settings-field mb-0">
                        <label class="clinic-label" for="apeBatchCapacity">Patient Limit</label>
                        <input class="settings-input" id="apeBatchCapacity" name="capacity" type="number" min="1" max="5000" value="50" required>
                        <p class="settings-help mb-0">The combined size of selected groups cannot exceed this limit.</p>
                    </div>
                    <div class="settings-field mb-0">
                        <label class="clinic-label" for="apeBatchStart">Start Time</label>
                        <input class="settings-input" id="apeBatchStart" name="start_time" type="time" required>
                    </div>
                    <div class="settings-field mb-0">
                        <label class="clinic-label" for="apeBatchEnd">End Time</label>
                        <input class="settings-input" id="apeBatchEnd" name="end_time" type="time" required>
                    </div>
                </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 space-y-4">
                    <div>
                        ...
                        <input class="settings-input" id="apeBatchSearch" type="search" placeholder="Search a section, department, or office">
                    </div>
    ...
    <script>
        (() => {
            ...
            if (!modal || !form || !category) return;

            const activeRows = () => {
                const term = (search.value || '').trim().toLowerCase();
                return rows.filter((row) => {
                    if (row.dataset.category !== category.value) return false;
            ...
            const selectedPatientTotal = () => checkboxes.reduce((total, box) => {
                ...
                const row = box.closest('[data-ape-group]');
                return total + Number(row?.dataset.patientCount || 0);
            }, 0);
            ...
            category.addEventListener('change', () => {
                checkboxes.forEach((box) => {
                    const row = box.closest('[data-ape-group]');
                    if (row?.dataset.category === previousCategory) box.checked = false;
                });

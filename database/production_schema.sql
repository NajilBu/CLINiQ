...
CREATE TABLE patient_registration_verifications (
...
CREATE TABLE student_school_year_enrollments (
...
CREATE TABLE school_employees (
  ...
  role_classification ENUM('Faculty', 'Non-Teaching Personnel') NOT NULL,
...
CREATE TABLE ape_schedule_batches (
  ...
  patient_category ENUM('Student') NOT NULL DEFAULT 'Student',
...
CREATE TABLE ape_records (
  ...
  entry_mode ENUM('Student Scheduled', 'Clinic Manual') NOT NULL DEFAULT 'Student Scheduled',
  patient_id BIGINT UNSIGNED NOT NULL,
  academic_year VARCHAR(20) NOT NULL,
  exam_date DATE NULL,
...
-- Failed login identifiers and IP addresses are stored only as keyed hashes.

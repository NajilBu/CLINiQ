...
RUN sed -i 's/\r$//' /usr/local/bin/cliniq-entrypoint /var/www/html/scripts/backup/container_scheduler.sh /var/www/html/scripts/email/worker.sh \
    && chmod 0755 /usr/local/bin/cliniq-entrypoint /var/www/html/scripts/backup/container_scheduler.sh /var/www/html/scripts/email/worker.sh \

...
RUN sed -i 's/\r$//' /usr/local/bin/cliniq-entrypoint /var/www/html/scripts/backup/container_scheduler.sh \
    && chmod 0755 /usr/local/bin/cliniq-entrypoint /var/www/html/scripts/backup/container_scheduler.sh \

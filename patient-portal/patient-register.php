...
<div class="student-field"><label class="student-label" for="registration_email">Email Address</label><input class="student-input" id="registration_email" name="email" type="email" autocomplete="email" value="<?= student_e((string) ($_POST['email'] ?? '')) ?>" required></div>
<div class="student-field"><label class="student-label" for="email_confirmation">Confirm Email Address</label><input class="student-input" id="email_confirmation" name="email_confirmation" type="email" autocomplete="email" value="<?= student_e((string) ($_POST['email_confirmation'] ?? '')) ?>" required></div>
...
<div class="student-field"><label class="student-label" for="first_name">First Name</label><input class="student-input" id="first_name" name="first_name" value="<?= student_e((string) ($values['first_name'] ?? '')) ?>" required></div><div class="student-field"><label class="student-label" for="middle_name">Middle Name (optional)</label><input class="student-input" id="middle_name" name="middle_name" value="<?= student_e((string) ($values['middle_name'] ?? '')) ?>"></div><div class="student-field"><label class="student-label" for="last_name">Last Name</label><input class="student-input" id="last_name" name="last_name" value="<?= student_e((string) ($values['last_name'] ?? '')) ?>" required></div><div class="student-field"><label class="student-label" for="birthdate">Birthdate</label><input class="student-input" id="birthdate" name="birthdate" type="date" max="<?= date('Y-m-d') ?>" value="<?= student_e((string) ($values['birthdate'] ?? '')) ?>" required></div>
...
<div class="student-field"><label class="student-label" for="registration_password">Password</label><div class="relative"><input class="student-input pr-14" id="registration_password" name="password" type="password" autocomplete="new-password" minlength="8" required><button type="button" class="student-toggle-pw" data-target="registration_password">Show</button></div></div><div class="student-field"><label class="student-label" for="password_confirmation">Confirm Password</label><div class="relative"><input class="student-input pr-14" id="password_confirmation" name="password_confirmation" type="password" autocomplete="new-password" minlength="8" required><button type="button" class="student-toggle-pw" data-target="password_confirmation">Show</button></div></div></div>
...
<script>document.querySelectorAll('.student-toggle-pw').forEach((button)=>button.addEventListener('click',()=>{const input=document.getElementById(button.dataset.target);if(!input)return;input.type=input.type==='password'?'text':'password';button.textContent=input.type==='password'?'Show':'Hide';}));document.getElementById('verification_code')?.addEventListener('input',(event)=>{event.target.value=event.target.value.replace(/\D/g,'').slice(0,6);});</script>

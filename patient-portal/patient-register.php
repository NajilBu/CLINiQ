...
<div class="student-note"><span class="material-symbols-outlined">info</span><div>Your account starts as Applicant. Passport and appointment access unlock after final APE clearance.</div></div>
<label class="flex items-start gap-3 text-xs font-bold text-slate-600 mt-4"><input class="mt-0.5" type="checkbox" name="legal_acknowledgement" value="1" required><span>I have read and acknowledge the <a href="<?= student_e(student_legal_url('terms')) ?>" target="_blank" rel="noopener" class="student-auth-link">Terms of Use</a> and <a href="<?= student_e(student_legal_url('privacy')) ?>" target="_blank" rel="noopener" class="student-auth-link">Privacy Notice</a>, including how CLINiQ processes health information.</span></label>
<button class="student-button w-full" type="submit">Create Applicant Account <span class="material-symbols-outlined">person_add</span></button></form>
<?php endif; ?>

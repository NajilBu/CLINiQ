...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $selectedId = filter_var($_POST['selected_visit_id'] ?? null, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
        $selected = $selectedId ? clinic_feedback_visit_for_person($db, $personId, $selectedId) : null;
        if (!$selected || $selected['status'] !== 'Completed' || clinic_feedback_already_sent($db, (int) $selected['visit_id'])) {
            throw new InvalidArgumentException('That completed visit is no longer available for feedback.');
        }
        $values = $_POST;
        clinic_feedback_submit($db, ['person_id' => $personId, 'visit_id' => (int) $selected['visit_id']], $_POST);
        patient_notification_mark_source_read($db, $personId, 'clinic_feedback', (int) $selected['visit_id']);
...
<div class="student-feedback-page">
    ...
    <?php elseif (!$pendingVisits): ?>
        <section class="student-card student-card-pad">
            <h2>No feedback due</h2>
            <p class="student-feedback-muted">You do not have a completed clinic visit waiting for feedback.</p>
            <a class="student-button student-button-secondary" href="patient-dashboard.php">Back to dashboard</a>
        </section>
    <?php else: ?>
        ...
        <?php if ($selectedVisit): ?>
            ...
            <form method="post" id="student-feedback-form" class="student-feedback-form">
                ...
                <input type="hidden" name="selected_visit_id" value="<?= (int) $selectedVisit['visit_id'] ?>">
                <section class="student-card student-card-pad">
                    ...
                    <div class="student-feedback-privacy">
                        ...
                        <details>
                            ...
                            <div>
                                <p>We collect your visit details, ratings, comments, and consent record for service quality improvement, accountability, and required clinic reporting. Your response is linked to this completed visit and is not anonymous.</p>
